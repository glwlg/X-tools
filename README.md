...
-   **截图贴图**：全局快捷键 `Alt + S/C/A` 唤起截图，支持区域选取、标注绘制、贴图钉在桌面、OCR 文字识别、二维码识别。
...
| Hash 计算 | `hash` | 计算文本的 MD5/SHA 等哈希值 |
| JSON 格式化 | `json` | JSON 美化与压缩 |
| URL 编解码 | `url` | URL 编码与解码 |
| UUID 生成 | `uuid` | 生成随机 UUID |
| 二维码生成 | `qr` | 输入文本生成二维码贴图 |
| Hosts 管理 | `hosts` | 打开 Hosts 管理窗口（直接操作） |
...
-   **OCR 文字识别**：基于 RapidOCR 离线识别截图中的文字，支持选中复制。

...
-   **JSON 对比**：打开独立窗口对比两个 JSON，支持语义差异、规范化行差异和报告复制。
...
| JSON 对比 | `jd` / `jsondiff` / `jsoncompare` / `json对比` | 打开 JSON 对比窗口，查看语义差异与规范化 Diff |
...
-   **管理入口**：设置页 `宏` 子页面可对工作流做新增、编辑、删除和步骤排序，保存后立即生效。
-   **可视化编辑**：每一步以卡片形式展示，包含执行命令、结果匹配、变量提示和上移/下移/删除操作；下方会实时生成兼容配置的命令文本预览。
-   **文本格式**：底层仍按每行一步保存，使用 `command | pick`（`pick` 可省略）。
    -   `command`：按“触发词 + 参数”执行插件命令，例如 `url {clipboard}`。
    -   `pick`：按结果名称前缀匹配目标项，例如 `MD5`、`编码结果`。
-   **变量支持**：
    -   `{clipboard}`：当前剪贴板文本。
    -   `{prev}`：上一步选中结果的 `path`。
...
-   **配置总览**：通用页顶部集中展示开机启动、主题、截图动作、快捷键、宏和插件状态。
-   **外观配置**：主题配置从 JSON 编辑改为颜色卡片和颜色选择器，修改后实时预览并立即保存。
-   **截图保存预览**：文件名模板支持 `{date}`、`{time}`、`{datetime}` 插入按钮，能即时看到最终保存路径。
-   **快捷键看板**：录制快捷键时先在看板中预览，确认后点击保存使全局快捷键生效。
-   **诊断卡片**：性能诊断页用指标卡展示最近耗时、平均值、p95 和调用次数，原始文本仍保留给排查使用。
...
当前主技术路线继续保持 **Python + PyQt6**，不进行前端壳重写。跨平台能力通过 `src/platform` 适配层逐步补齐，现有 Windows 行为保持不变：
-   `src/platform/file_search.py`：文件搜索能力，当前 Windows 使用 Everything，其他平台先降级为空结果。
-   `src/platform/applications.py`：应用扫描能力，当前 Windows 扫描开始菜单快捷方式。
-   `src/platform/hotkeys.py`：全局快捷键能力，当前 Windows 使用 `RegisterHotKey`。
-   `src/platform/startup.py`：开机启动能力，当前 Windows 写入用户注册表。
-   `src/platform/hosts.py`：Hosts 文件路径与提权写入能力，当前 Windows 通过 PowerShell UAC 写入。
-   `src/platform/shell.py`：统一打开文件、目录、URL，替代业务代码直接调用 `os.startfile`。

插件可通过 `supported_platforms` 和 `required_capabilities` 声明平台约束。`PluginManager` 默认只返回当前平台可用的插件，后续新增 macOS / Linux 实现时，只需要补齐对应适配层能力并调整能力表。
...
6. 可选：配置 `supported_platforms` / `required_capabilities`，让插件只在具备对应平台能力时启用。
7. 重启应用，插件将自动激活。
